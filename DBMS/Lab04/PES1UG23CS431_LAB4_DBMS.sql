...
INSERT INTO event VALUES ('E34','AI Hackathon','Seminar Hall',2,205,900.00,'T4');
...
INSERT INTO purchased VALUES ('P1017','S6','Fish Tacos','2025-07-10 14:00:00',3);
...
SELECT SRN FROM registration WHERE event_id = 'E2'
     ...
     EXCEPT
     SELECT SRN FROM registration WHERE SRN IN (
     SELECT SRN FROM registration WHERE event_id='E2'
     INTERSECT
     SELECT SRN FROM registration WHERE event_id='E5');

SELECT p.SRN, p.name as participant_name, v.name as visitor_names, COUNT(v.SRN) AS visitor_count
     FROM participant p
     LEFT JOIN visitor v ON p.SRN = v.SRN
     GROUP BY p.SRN, p.name, v.name;
...
SELECT e.event_name, CASE WHEN p.date_of_conduction > '2046-12-31' THEN 1 ELSE 0 END AS After_Golden_Jublie
     FROM event e JOIN event_conduction p ON e.event_id=p.event_id;
